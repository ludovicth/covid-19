...
contamination <- function(pop, cluster, contag, prob_sain, prob_contact, n_contact){
  ...
  population[["sains"]]["0"] <- population[["sains"]]["0"] + n_contam * prob_sain
  population[["non_decl"]]["0"] <- population[["non_decl"]]["0"] + n_contam * (1 - prob_sain)
...
evolution <- function(pop, d, g, prob_grav, satur_hop, tx_mort){
  ...
  names(pop[["immu"]]) <- NULL
  spy <<- taille_pop
  spy2 <<- sum(sapply(pop, sum)) 
  # Etape 7 : Vérification
  if(abs(sum(sapply(pop, sum)) - taille_pop) > 10^-8) stop ("La taille de la population en entrée est différente de celle en sortie")
